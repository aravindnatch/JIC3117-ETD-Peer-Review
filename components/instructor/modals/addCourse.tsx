...
export default function AddCourse({ userData, setAddCourse }: ModalProps) {
  const [ cookies ] = useCookies(['userData']);
  const [ name, setName ] = useState('')
  const [ pressed, setPressed ] = useState(false)
  const [ selectedOption, setSelectedOption ] = useState(userData.questions[0]?._id || "")
    ...
    <div onClick={(event) => closeModal(event, setAddCourse)} className="fixed z-10 flex justify-center items-center p-4 inset-0 h-full bg-[rgba(0,0,0,0.3)] h-modal overflow-x-hidden overflow-y-auto">
      <div className="fixed w-full max-w-md h-auto">
        <div className="relative bg-white rounded-lg">
          <div onClick={() => setAddCourse(false)} className="cursor-pointer absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center ">
          ...
          <div className="px-6 py-6 lg:px-8">
            ...
            <div>
              ...
              <div>
                <select value={selectedOption} onChange={e => setSelectedOption(e.target.value)} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                  {userData.questions?.map((o: any, i: any) => (
            ...
            <button onClick={handleAdd} disabled={name == "" || selectedOption == ""} className={`w-full text-white bg-indigo-700 hover:bg-indigo-800 focus:ring-4 focus:outline-none focus:ring-indigo-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center disabled:opacity-40`}>
